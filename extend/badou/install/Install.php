...
class Install
{
    ...
    public static string $buildConfigFileName = 'badouadmin.php';
    /**
     * 安装完成标记
     * 配置完成则建立lock文件
    ...
    {
        $this->installPath = root_path() . 'extend' . DIRECTORY_SEPARATOR . 'badou'  . DIRECTORY_SEPARATOR . 'install' . DIRECTORY_SEPARATOR;
        $this->app = app();
    }
    ...
    public function index()
    {
        ...
        if ($this->request->isPost()) {
            ...
            return $output(1, __('Install Successed'), null, ['adminName' => 'admin.php']);
        }
        $errInfo = '';
        try {
            $this->checkenv();
    ...
    public function baseConfig(): void
    {
        ...
        $sql = file_get_contents($this->installPath . 'badouadmin-install.sql');
        ...
        // 设置新的Token随机密钥key
        $oldTokenKey        = Config::get('badouadmin.token.key');
