...
layui.use(["layer", "badou", "toast"], function () {
  ...
  function gettablecolumnbutton(options) {
    ...
    if (
      typeof options.tableId !== "undefined" &&
      typeof options.buttonIndex !== "undefined"
    ) {
      ...
      if (tableOptions) {
        ...
        $.each(tableOptions.cols, function (i, columns) {
          $.each(columns, function (j, column) {
            if (
              typeof column.field !== "undefined" &&
              column.field === options.field
            ) {
              columnObj = column;
              return false;
            }
  ...
  $(document).on("click", ".btn-dialog,.dialogit", function (e) {
    ...
    if (typeof options.confirm !== "undefined") {
      ...
      window[$(that).data("window") || "self"].layui.badou.api.open(
        url,
        title,
        options,
      );
  ...
  $(document).on("click", ".btn-captcha", function (e) {
    ...
    if (["mobile", "email"].indexOf(type) > -1) {
      var element = $(this).data("input-id")
        ? $("#" + $(this).data("input-id"))
