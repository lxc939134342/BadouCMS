...
/**
 * 重复定义 在 common.php 中定义了 library\AdminAuth.php
 */
class Auth extends \badou\Auth
{
    //当前登录会员详细信息
    protected $error        = '';
    ...
    public function getChildrenGroupIds(bool $withself = false)
    {
        ...
        // 取出所有分组
        $groupList = \app\admin\model\AdminGroup::where('status', 1)->select()->toArray();
        $objList   = [];
        foreach ($groups as $k => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function getChildrenAdminIds(bool $withself = false)
    {
        ...
        if (!$this->isAdministrator()) {
            ...
            $childrenAdminIds = \app\admin\model\AdminGroupAccess::where('group_id', 'in', $groupIds)->column('uid');
        } else {
            //超级管理员拥有所有人的权限
    ...
    public function getSidebar($params = [])
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = \app\admin\model\AdminRule::where('status', 1)
            ->where('ismenu', 1)
            ->order('listorder', 'desc')
            ->cache("__menu__")
            ->select()->toArray();
        $indexRuleList = \app\admin\model\AdminRule::where('status', 1)
            ->where('ismenu', 0)
