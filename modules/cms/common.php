...
if (!function_exists('get_mapping')) {
    // 获取数据数组的映射数组
...
if (!function_exists('escape_string')) {
    // 获取转义数据，支持字符串、数组、对象
    // 注意：ThinkORM 已通过参数绑定防止SQL注入，此处仅做XSS过滤即可
...
if (!function_exists('mark')) {
    function mark($string)
    {
        if ($reqdata = request()->param('keyword')) {
            // 安全处理：先转义正则特殊字符防止注入，再过滤HTML标签防止XSS
            $keyword = preg_quote(strip_tags($reqdata), '/');
            if ($keyword !== '') {
...
if (!function_exists('result_value_search')) {
    // 结果集中查找指定字段父节点是否存在
...
if (!function_exists('implode_quot')) {
    // 数组转换为带引号字符串
...
if (!function_exists('strlen_both')) {
...
if (!function_exists('network_match')) {
    // 判断是否在子网
...
if (!function_exists('get_auto_code')) {
    // 获取字符串型自动编码
...
if (!function_exists('get_backend_lang')) {
    /**
     * 获取后台语言
     * @return string
     */
...
if (!function_exists('get_frontend_lang')) {
    /**
     * 获取前台语言
     * @return string
     */
...
if (!function_exists('set_backend_lang')) {
    /**
     * 设置后台语言
     * @param string $lg
     * @return void
...
if (!function_exists('set_forntend_lang')) {
    /**
     * 设置前台语言
     * @param string $lg
     * @return void
...
if (!function_exists('resize_img')) {
    ...
    function resize_img(string|null $src_image, int $max_width = 0, int $max_height = 0, int $img_quality = 90): string
    {
        ...
        // 安全处理：过滤路径穿越字符，防止读取公共目录以外的文件
        $src_image = str_replace(['../', '..\\'], '', $src_image);
        $src_image = ltrim($src_image, '/');
        /* 原图片物理路径 */
