KindEditor.ready(function (K) {
  layui.use(["badou"], function () {
    ...
    var upload = function (editorInstance, file, uploadCallback) {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          var msg =
            (xhr.responseJSON ? xhr.responseJSON.msg : error) || "上传失败";
          if (uploadCallback) {
            // 如果是 KindEditor 内部上传按钮触发，传递错误信息
      ...
      var click = function () {
        var html = [
          ...
          '<div class=""><div class="ke-inline-block ke-upload-button">' +
            '<form class="ke-upload-area ke-form nice-validator n-default" method="post" enctype="multipart/form-data" style="margin:50px auto;">' +
    ...
    function loadEditor() {
      // 只对未初始化的textarea创建KindEditor实例
      $("textarea.editor:not([data-editor-initialized])").each(function () {
        var textarea = $(this);
        K.create(textarea[0], {
          ...
          fileManagerJson: badou.api.fixurl(
            "general.attachment/select?element_id=&multiple=true&mimetype=*",
          ), // 文件管理器功能保持不变
          themeType: "black", //主题
          width: "100%",
          newlineTag: "br", // 使用 br 作为换行标签，避免自动添加 <br>
          filterMode: false, // 关闭 HTML 过滤模式，避免自动格式化
          wellFormatMode: false, // 关闭美化格式模式，避免自动添加空标签
          items: [
            "source",
            "undo",
            "redo",
          ...
          afterCreate: function () {
            var self = this;
            // 标记textarea为已初始化
            textarea.attr("data-editor-initialized", "true");
            //Ctrl+回车提交
            K.ctrl(document, 13, function () {
              ...
              if (self.srcElement && self.srcElement[0]) {
                ...
                console.warn(
                  "KindEditor: srcElement[0] is undefined, cannot submit form.",
                );
              }
            });
          ...
          beforeUpload: function (ke_upload_button_callback, file) {
            ...
            var file = file
              ? file
              : $("input.ke-upload-file", this.form).prop("files")[0];
